window.Travian = {
    ...
    ajax: function(options) {
        ...
        if (options.data) {
            options.data.ajaxToken = typeof Travian.advisableDecriminalizationTorchesPummeled === "function" ? Travian.advisableDecriminalizationTorchesPummeled() : undefined;
            if (options.data.cmd) {
    ...
    insertScript: (function() {
        ...
        return function(c) {
            ...
            a.push(c);
            var d = jQuery("<script>").attr("type", "text/javascript").attr("id", (c.id ? c.id : undefined)).attr("src", c.src).attr("defer", !!c.defer);
            if (false) {
                d[0].onreadystatechange = function() {
                    if (d[0].readyState === "loaded" || d[0].readyState === "complete" || d[0].readyState === 4) {
                        c.onLoad(true, d[0])
                    }
                }
            } else {
                d[0].onload = c.onLoad.apply(d[0], [true])
            }
    ...
    findButtonsAndBindEvents: function(a) {
        var b = a ? jQuery(a).find("button") : jQuery("button");
        b.each(function(d, f) {
            var e = jQuery(f);
            var c = e.find(".addHoverClick");
            if (c === null) {
                c = e
            }
            Travian.addMouseEvents(f, c)
        })
    },
...
jQuery(window).on("domAltered", function(b, a) {
...
Travian.Dialog.Dialog = function(a) {
    ...
    this.correctDialogPosition = function(b) {
        var g = Travian.WindowManager.getWindowDimensions(),
            f = this.wrapper.outerWidth(),
            c = 200,
            e = b.left,
            d = b.top;
        if (e < 0) {
    ...
    this.render = function() {
        var f = this;
        var b = jQuery("body");
        var g = jQuery("<div/>").html(this.buttonTemplates.button);
        var d = g.find("button").first();
        if (this.options.savePositionForSession.preferenceKey) {
            var e = Travian.Game.Preferences.get(this.options.savePositionForSession.preferenceKey);
            if (e !== null) {
                e = JSON.parse(e);
                this.options.savePositionForSession.position = e.position
            }
        }
        if (!d) {
        ...
        d.addClass("green");
        d.addClass("ok");
        d.addClass("dialogButtonOk");
        d.attr("type", "submit");
        var c = g.find("div.button-content");
        c.prepend(jQuery('<div class="text"></div>'));
        this.wrapper = jQuery('<div class="dialogWrapper"></div>').css({
            position: "absolute",
            opacity: 0
        });
        this.wrapper.html('<div class="dialog ' + this.options.cssClass + '"><div class="dialog-container"><div class="dialog-background-tl"></div><div class="dialog-background-tr"></div><div class="dialog-background-tc"></div><div class="dialog-background-ml"></div><div class="dialog-background-mr"></div><div class="dialog-background-mc"></div><div class="dialog-background-bl"></div><div class="dialog-background-br"></div><div class="dialog-background-bc"></div><div class="dialog-contents"><form action="?" method="get" accept-charset="UTF-8"><div class="dialog-dragbar"><div class="dragbar-inner-left"></div><div class="dragbar-inner-mid"></div><div class="dragbar-inner-right"></div></div><div class="iconButton small info"></div><div id="dialogCancelButton" class="iconButton small cancel"></div><div class="title"></div><div class="content" id="dialogContent"></div><div class="buttons">' + g.html() + "</div></form></div></div></div>");
        b.prepend(this.wrapper);
        jQuery(window).resize(function() {
            f.updatePosition(200, true);
            f.triggerPositionsSync()
        });
        ...
        if (this.options.draggable) {
            ...
            new Travian.Moveable(this.wrapper, {
                ...
                onDrop: function(i) {
                    var h = jQuery(i).position();
                    f.options.dragPosition = f.correctDialogPosition(h);
                    if (f.options.savePositionForSession.preferenceKey !== null) {
                        f.options.savePositionForSession.position = f.correctDialogPosition(h)
                    }
                },
                onDrag: function(i) {
                    var h = jQuery(i);
                    h.css(f.correctDialogPosition(h.position()));
                    f.options.onDrag(f);
                    f.triggerPositionsSync()
                }
            });
        ...
        this.form = this.wrapper.find("form").on("submit", function(h) {
            if (f.form.disabled) {
                h.stopPropagation();
                return false
            }
            f.disableForm();
            f.options.onOkay(f, f.content);
            if (f.options.keepOpen === false) {
                f.close()
            }
            if (f.options.preventFormSubmit) {
                h.stopPropagation();
                return false
            }
        });
        ...
        this.buttonOkText = this.wrapper.find("button.ok .text");
        if (this.buttonOkText.length > 0) {
            this.buttonOkText.html(this.options.buttonTextOk)
        } else {
            Travian.Tip.set(this.buttonOk, this.options.buttonTextOk)
        }
        if (this.options.buttonCloseOnClickOk) {
            this.buttonOk.click(function(h) {
                f.close()
            })
        }
        this.buttonCancel = this.wrapper.find(".cancel").click(function(h) {
            f.close()
        });
        if (this.options.buttonCancel === false) {
            this.buttonCancel.hide()
        }
        this.buttonCancelText = this.wrapper.find(".cancel .text");
        if (this.buttonCancelText.length > 0) {
            this.buttonCancelText.html(this.options.buttonTextCancel)
        } else {
            Travian.Tip.set(this.buttonCancel, this.options.buttonTextCancel)
        }
        if (f.options.enableBackground) {
            this.overlay = new Travian.Dialog.DarkBackgroundOverlay(document.body, {
                open: {
                    opacity: (f.options.darkOverlay) ? 0.8 : 0.3
                },
                duration: this.options.fx.duration,
                animateFunction: this.animateFunction,
                onClick: function() {
                    if (f.options.overlayCancel) {
                        f.close()
                    }
                }
            })
...
Travian.Dialog.Dialog.prototype.close = function() {
    var b = this;
    this.open = false;
    ...
    this.animateFunction(this.wrapper, this.options.fx.close, this.options.fx.duration, a);
    if (this.overlay) {
        this.overlay.close()
    }
...
var Digitarald = {
    AutoCompleter: function(b, a) {
        ...
        this.setSelection = function(j) {
            var k = this.selected.attr("value"),
                l = k;
            ...
            if (h === "object" && !k.length) {
                ...
                for (var g = 0; g < k.length; g++) {
                    ...
                    if (this.options.injectChoice) {
                        ...
                        var e = jQuery("<li />").attr("value", f).html(j);
        ...
        this.choiceOver = function(g, h) {
            ...
            this.selectedValue = this.selected.attr("value");
            if (this.overflown) {
...
Travian.Game = {
    ...
    showEditVillageDialog: function(e, a, c, b) {
        var d = jQuery("div#villageNameField").html();
    ...
    toggleBox: function(f, c, h) {
        ...
        for (var e = 0; e < j.length; e++) {
            ...
            if (a !== b) {
                d = a;
                f.addClass(d);
                Travian.Tip.set(f.find("button.toggle"), Travian.Translation.get("{" + h + "_" + d + "}"))
            }
        }
...
Travian.Game.ColorPicker = function(a, b) {
    ...
    this.draw = function() {
        ...
        this.options.colors.forEach(function(d) {
            ...
            jQuery("<div/>").addClass("entry").html("&nbsp;").css({
                "background-color": d
            }).attr("colorPick", d).appendTo(e)
        });
        return this
    };
    this.options = Object.assign({}, {
...
Travian.Game.Menu = function(a) {
    this.getMorphContainer = function(g) {
        var f = jQuery(g.target);
        var d = f.parents(".container");
        return d.length > 0 ? d : f
    };
    var c = jQuery(a);
    var b = this;
    c.find("a").each(function(d, g) {
        var f = jQuery(g).parents(".normal");
        if (f.length === 0) {
            return
        }
        f.on({
            mouseenter: function(h) {
                b.getMorphContainer(h).toggleClass("hover")
            },
...
Travian.Game.AddLine = function(a) {
    ...
    this.options.elements.add.on("click", function(g) {
        ...
        b.options.elements.insert.after(f.css({
            opacity: 0
        }));
        f.animate({
            opacity: 1
...
Travian.Game.AutoCompleter.VillageName = function(a) {
    Travian.Game.AutoCompleter.call(this, a, "villagename", {
        maxChoices: 20
    })
};
        ...
        if (!Travian.Game.Messages.check_in_progress) {
            ...
            Travian.ajax({
                ...
                onError: function(c, e) {
                    var d = new Travian.Dialog.Dialog({
                        preventFormSubmit: true
                    });
                    d.setContent(e);
                    d.show()
...
Travian.Game.BBEditor = function(b, a) {
    ...
    this.insertTag = function(g) {
        var f = this;
        g.preventDefault();
        this.hidePreview();
        var d;
        if (jQuery(g.target).is("button")) {
            d = jQuery(g.target)
        } else {
            d = jQuery(g.target).parent()
        }
        ...
        switch (Travian.Game.BBEditor.getInformationFromClass(d[0], "bbType")) {
            case "d":
                f.insertAroundCursor(this.textArea, "[" + c + "]", "[/" + c + "]");
                break;
            case "s":
                f.insertAtCursor(this.textArea, c);
                break;
            case "o":
                f.insertAtCursor(this.textArea, ("[" + Travian.Game.BBEditor.getInformationFromClass(d[0], "bbTag") + "]"));
                break
        }
    };
...
Travian.Game.RaidList = {
    ...
    dialog: function(a, c) {
        var b = new Travian.Dialog.Dialog(Object.assign({
            preventFormSubmit: true
        }, a));
        b.setContent(c);
        b.show()
    ...
    saveSlot: function(b, f, a, c) {
        var e = this;
        c = Travian.Game.RaidList.weAreAdding ? null : (c || null);
        var d = jQuery("#raidListSlot #edit_form .troops");
        ...
        Travian.ajax({
            ...
            onError: function(h, g) {
                jQuery('<div class="error">' + g + "</div>").css("marginBottom", "1em").insertAfter(d)
            }
        })
    },
...
Travian.Game.Hero.Inventory = function(a) {
    ...
    this.createDivs = function() {
        ...
        var d = function(h) {
            var g = Travian.isMobile() || e.DoubleClickPreventer === null || !e.DoubleClickPreventer.prevent;
            if (h.isUseable && g) {
        ...
        this.makeDraggable();
        this.options.elementHeroBody[0].src = String(this.options.urlBodyImage).replace((/\\?\{([^{}]+)\}/g), jQuery.proxy(this.substitute, {
            heroBodyHash: e.options.heroBodyHash
        }));
        return this
    };
    ...
    this.executeMovement = function(e, c, b) {
        var d = this;
        if (!d.DoubleClickPreventer) {
            d.DoubleClickPreventer = new Travian.DoubleClickPreventer();
            d.DoubleClickPreventer.timeout = 5000
        }
        if (!d.DoubleClickPreventer.check()) {
            return false
        }
        Travian.ajax({
            data: {
    ...
    this.moveToDrop = function(h, e, c) {
        ...
        if (c !== true && !this.isInventoryId(e.attr("id")) && ((h[0].item.instant && h[0].item.amount === 1) || h[0].item.amount > 1)) {
            ...
            if (g.show) {
                g.onOkay = function(j, k) {
                    f(j, k);
                    return true
                };
                g.onClose = function() {
                    i.moveToDrop(h, h[0].item.placeElement, true);
                    return true
                };
                this.dialog(g)
            }
...
Travian.Game.Hero.SilverExchange = function(a) {
    ...
    this.overrideGoldAndSilver = function(b, e, h, g) {
        ...
        jQuery(".ajaxReplaceableSilverAmountDiff").each(jQuery.proxy(f, {
            type: "silver"
        }));
        jQuery(".ajaxReplaceableGoldAmountDiff").each(jQuery.proxy(f, {
            type: "gold"
        }));
...
Travian.Game.Hero.PropertySetter = function(b, a) {
    ...
    this.update = function() {
        jQuery.each(this.options.attributes, function(c, d) {
            d.updateButtons()
        });
        this.options.elementAvailablePoints.html(this.getAvailablePoints());
        this.onChange();
        return this
    };
    this.initialize(b)
...
Travian.Game.Activation = function() {
    ...
    this.updateArrowPositions = function() {
        ...
        for (var a = 0; a < b.length; a++) {
            if (b[a].checked === true) {
                TweenLite.to(jQuery("#presentation svg .inner, #presentation svg .outer"), 0.25, {
...
Travian.Game.Highlight.Renderer.Rectangle = function(b, d) {
    ...
    this.activate = function() {
        ...
        if (e.options.draggable) {
            $(document).off("dialogDraggingSync").on("dialogDraggingSync", {
                renderer: e
            }, c)
        }
        return this
...
Travian.Game.Marketplace.ExchangeResources = {
    ...
    calculateRest: function() {
        var f = document.getElementsByName("m2[]");
        this.overall = 0;
        var e;
        for (var c = 0; c < f.length; c++) {
            var b = "";
            for (var a = 0; a < f[c].value.length; a++) {
                if ((f[c].value.charAt(a) >= "0") && (f[c].value.charAt(a) <= "9")) {
                    b = b + f[c].value.charAt(a)
                }
            }
            if (b === "") {
                b = "0";
            ...
            var d = e - (parseInt(document.getElementById("org" + c).innerHTML) || 0);
            document.getElementById("diff" + c).innerHTML = "" + (d > 0 ? "+" + d : d);
            this.overall += e
        }
        document.getElementById("newsum").innerHTML = this.overall;
        document.getElementById("remain").innerHTML = "" + ((parseInt(document.getElementById("org4").innerHTML) || 0) - this.overall);
        this.testSum()
    },
    fillup: function(a) {
        var b = document.getElementsByName("m2[]");
        if (a < 3) {
            b[a].value = this.max123
        } else {
            b[a].value = this.max4
        }
        this.calculateRest()
    ...
    portion: function(c) {
        ...
        Travian.ajax({
            ...
            onSuccess: function(e) {
                a.each(function(f, g) {
                    g.value = e.distributed[f];
                    jQuery("#org" + f).text(e.resources[f]);
                    jQuery("input#m1\\[" + f + "\\]").val(e.resources[f])
                });
                jQuery("table#npc #org4").text(e.resources.reduce(function(g, f) {
                    return g + parseInt(f)
                }, 0));
                d.calculateRest()
            },
            onError: function(e, f) {
                (new Travian.Dialog.Dialog()).setContent(f).show()
            }
        });
        return false
    },
...
Travian.Game.AllianceDonation = {
    ...
    donate: function(a, c, b) {
        var d = jQuery("#donate_gold_confirm");
        if (c !== "donate_gold") {
    ...
    initBonusOverview: function() {
        ...
        if (a.length > 0) {
            a.each(function(e, d) {
                jQuery(d).on("click", function(i) {
                    var h = jQuery(this).attr("ref");
                    var f = jQuery(this).children("img.openedClosedSwitch");
                    if (h.length > 0 && f.length > 0) {
                        var g = jQuery(".bonusInfo." + h);
...
Travian.Game.AllianceLeave = function(a) {
    ...
    this.request = function() {
        ...
        Travian.ajax({
            ...
            onFailure: function(d, c) {
                jQuery("#allianceLeavePopupError").innerHTML = c;
                return false
            }
        });
        return this
...
Travian.Game.VideoFeature = function(a) {
    this.request = function() {
        var c = this;
...
Travian.Game.VideoFeature.prototype = Object.create(Travian.Dialog.Ajax.prototype);
Travian.Game.VideoFeature.constructor = Travian.Game.VideoFeature;
...
Travian.Game.VideoFeatureEventListener = Object.create({
    ...
    bindEvents: function() {
        ...
        jQuery(window).on("startVideo", function(c, b) {
            if (!a.DoubleClickPreventer.check()) {
                return false
            }
            jQuery(window).on("message", function(d) {
                a.onMessage(d, b)
            });
            a.startVideo(b)
        })
    ...
    startVideo: function(a) {
        if (typeof a !== typeof undefined && typeof a.url !== typeof undefined) {
            ...
            jQuery("#videoArea").attr("src", a.url).show()
        }
    }
});
jQuery(function() {
                    ...
                    Travian.ajax({
                        ...
                        onSuccess: function(j) {
                            if (undefined !== j.reportingSuccessful && j.reportingSuccessful) {
                                ...
                                jQuery(".dialog button .button-container .button-content .text").html(j.closeButtonText);
...
Travian.Game.Village = {
    toggleBuildingLevels: function() {
        ...
        var a = jQuery("#levels").children();
        if (b) {
            a.find("div.labelLayer").each(function(d, f) {
                var h = jQuery(f);
                var g = h.html(),
                    e = h.parent();
                h.remove();
                e.html(g).removeClass("colorLayer")
            })
        } else {
            a.each(function(d, e) {
                var f = jQuery(e);
                var h = f.html(),
                    g = jQuery("<div/>").addClass("labelLayer").html(h);
                f.addClass("colorLayer").html("").append(g)
            })
        }
    }
};
...
Travian.Game.Map.Container = (function() {
    var b = function(d) {
        ...
        for (var k in d.keyboard) {
            if (d.keyboard.hasOwnProperty(k)) {
                var h = d.keyboard[k];
                if (typeof d.keyboard[k] === "string") {
                    d.keyboard[k] = {
                        fn: d.keyboard[k]
                    }
                }
                d.keyboard[k] = Object.assign({
                    periodical: 1
                }, d.keyboard[k]);
                if (typeof d.keyboard[k].fn === "string") {
                    g.keys[k] = false
                }
            }
        }
        var m = d.containerRender.css("cursor");
        var j = function(p, n, r, o) {
            if (!d.isEventsEnabled()) {
                return
            }
            var q = (p.which === 3 || p.button === 2);
            if (d.containerViewSize.x <= n && d.containerViewSize.y <= r && n <= d.containerViewSize.right && r <= d.containerViewSize.bottom && o === d.containerMover.get(0) && !q) {
                f = true;
                c = false;
                l = {
                    x: n,
        ...
        var e = function(o, n, q) {
            if (d.containerViewSize.x <= n && d.containerViewSize.y <= q && n <= d.containerViewSize.right && q <= d.containerViewSize.bottom) {
                d.currentMousePosition.browserAbsolute.x = n;
                d.currentMousePosition.browserAbsolute.y = q;
                d.currentMousePosition.browser.x = n - d.containerSize.x - d.elementSize.x;
                d.currentMousePosition.browser.y = q - d.containerSize.y - d.elementSize.y;
                d.currentMousePosition.map = d.transition.translateToMap(d.currentMousePosition.browser, {})
            } else {
                d.currentMousePosition.browserAbsolute.x = null;
                d.currentMousePosition.browserAbsolute.y = null;
                d.currentMousePosition.browser.x = null;
                d.currentMousePosition.browser.y = null;
                d.currentMousePosition.map.x = null;
                d.currentMousePosition.map.y = null
            }
            if (!f) {
                return
            }
            ...
            if ((Math.abs(p.x) + Math.abs(p.y)) > 0) {
                l = {
                    x: n,
                    y: q
                };
                c = true;
                d.containerRender.css({
                    cursor: "move"
                });
                d.move(p)
            }
        ...
        var i = function(o, n, r) {
            if (!d.isEventsEnabled()) {
                return
            }
            var q = (o.which === 3 || o.button === 2);
            if (n !== null && r !== null && d.containerViewSize.x <= n && d.containerViewSize.y <= r && n <= d.containerViewSize.right && r <= d.containerViewSize.bottom && !q && !c && f && !Travian.WindowManager.checkOpenWindowByContext("map")) {
                var p = d.transition.translateToMap({
                    x: n - d.containerViewSize.x - d.elementSize.x,
                    y: r - d.containerViewSize.y - d.elementSize.y
                }, {});
                if (d.tileDisplayInformation.type === "dialog") {
                    new Travian.Dialog.Ajax(Object.assign({}, d.tileDisplayInformation.optionsDialog, {
                        context: "map",
                        ...
                        onOpen: function(t, s) {
                            jQuery(s).find('a[href^="karte.php"]').on("click", function(v) {
                                v.preventDefault();
                                var u = Travian.parseURL(v.target.href);
                                d.moveTo({
                                    x: parseInt(u.searchObject.x || "0"),
                                    y: parseInt(u.searchObject.y || "0")
                                });
                                t.close();
                                return false
                            })
                    ...
                    Travian.popup(d.tileDisplayInformation.optionsPopup.url.substitute(p), d.tileDisplayInformation.optionsPopup.windowOptions)
                }
            }
            d.containerRender.css({
                cursor: m
            ...
            if (c) {
                o.stopPropagation();
                d.updateBrowserURL(d.transition.getPointOfCenterInView())
            }
            f = false;
            c = false
        };
        jQuery(window).on({
            ...
            wheel: function(n) {
                ...
                if (d.containerViewSize.x <= n.pageX && d.containerViewSize.y <= n.pageY && n.pageX <= d.containerViewSize.right && n.pageY <= d.containerViewSize.bottom && n.target === d.containerMover.get(0)) {
                    var o = d.transition.translateToMap({
                        x: n.pageX - d.containerViewSize.x - d.elementSize.x,
                        y: n.pageY - d.containerViewSize.y - d.elementSize.y
                    }, {});
                    if (n.originalEvent.deltaY > 0) {
                        d.zoomOut(o)
                    } else {
                        if (n.originalEvent.deltaY < 0) {
                            d.zoomIn(o)
            ...
            touchstart: function(n) {
                j(n, n.touches[0].pageX, n.touches[0].pageY, n.touches[0].target)
            },
            touchmove: function(n) {
                e(n, n.touches[0].pageX, n.touches[0].pageY)
            },
            touchend: function(n) {
                i(n, n.pageX, n.pageY)
            },
            keydown: function(n) {
                ...
                if (g.keys[n.keyCode] === false && n.target.nodeName.toLowerCase() !== "input") {
                    g.count++;
                    g.keys[n.keyCode] = true;
                    n.stopPropagation();
                    if (!g.fnTimer) {
                        g.fn = function() {
                            var s = 0;
                            for (var p in g.keys) {
                                if (g.keys.hasOwnProperty(p) && g.keys[p]) {
                                    var r = g.keys[p];
                                    if (r) {
                                        if (!d.keyboard[p]) {
                                            return
                                        }
                                        var u = d.keyboard[p].fn;
                                        if (u === false || !d[u]) {
                                            return
                                        }
                                        var q = "";
                                        if (u.substring(0, 4) === "move") {
                                            q = "normal";
                                            var o = d.keyboard.speed.slow;
                                            var t = d.keyboard.speed.fast;
                                            if (g[o] && !g[t]) {
                                                q = "slow"
                                            } else {
                                                if (!g[o] && g[t]) {
                                    ...
                                    s += d.keyboard[p].periodical
                                }
                            }
                            if (s > 0) {
                                g.fnTimer = requestAnimationFrame(g.fn)
                            }
                        };
                        if (d.keyboard[n.keyCode].periodical === 0) {
                            g.fn()
                        } else {
                            if (d.keyboard[n.keyCode].periodical > 0) {
                                g.fnTimer = requestAnimationFrame(g.fn)
            ...
            keyup: function(n) {
                ...
                if (g.keys[n.keyCode]) {
                    g.count--;
                    g.keys[n.keyCode] = false;
                    n.stopPropagation();
                    if (g.count === 0 && g.fnTimer) {
                        cancelAnimationFrame(g.fnTimer);
                        g.fnTimer = null;
                        d.updateBrowserURL(d.transition.getPointOfCenterInView())
                    }
                }
    ...
    var a = function(c) {
        ...
        this.positionChange = function() {
            var j = Object.assign(this.containerRender.getDimensions({
                computeSize: true
...
Travian.Game.Map.Updater = (function() {
    ...
    var c = function(f) {
        ...
        Object.keys(f.objects.images).map(function(h, g) {
            return {
                imageId: h,
                priority: f.objects.images[h].getPriority()
            }
        }).sort(function(h, g) {
    ...
    render: function(c, a) {
        ...
        jQuery(a).on("mousemove", function(g) {
            ...
            if (d === false) {
                d = {
                    title: "",
                    text: Travian.Helpers.substitute(b.tooltipHtml, c.transition.translateToMap(f))
                }
            }
...
Travian.Game.Map.Rulers = (function() {
    ...
    var f = function(g) {
        g.elements.moverX.css({
            backgroundImage: "url(" + Travian.Helpers.substitute(g.imgSource.x, {
                zoomLevel: g.transition.zoomLevel
            }) + ")"
        });
        g.elements.moverY.css({
            backgroundImage: "url(" + Travian.Helpers.substitute(g.imgSource.y, {
                zoomLevel: g.transition.zoomLevel
            }) + ")"
        })
    };
    ...
    var c = function(j, h, g) {
        ...
        j.elements.moverX.css({
            left: j.position.x
        });
        j.elements.moverY.css({
            top: j.position.y
        });
...
Travian.Game.Map.MiniMap = (function() {
    var c = function(i) {
        var e = i.transition.translateToMap({
            x: -i.mapContainer.elementSize.x,
            y: -i.mapContainer.elementSize.y
        }, {});
    ...
    var a = function(d, e) {
        ...
        this.animate = function() {
            ...
            if (this.expanded === false) {
                ...
                this.elements.container.animate({
                    height: this.elements.container._height.max
                }, 200);
                this.parentContainer.outline.update(this.elements.container.height() - this.elements.container._height.max)
            } else {
                this.expanded = false;
                this.cookie.set("minimap-expanded", false);
                this.elements.headlineExpander.removeClass("collapse").addClass("expand");
                this.elements.container.animate({
                    height: this.elements.container._height.min
                }, 200);
                this.parentContainer.outline.update(this.elements.container.height() - this.elements.container._height.min)
            }
            return this
        };
        this.getContentForTooltip = function(f, h) {
            var g = b(h, this);
            return {
                text: Travian.Helpers.substitute(this.tooltipHtml, g)
            }
        };
        this.render = function(h) {
            var k = this;
            this.container = jQuery(this.container).css({
                overflow: "hidden"
            }).prop("unselectable", "on").on("click", function(i) {
                k.mapContainer.moveTo(b(i, k))
            });
            Travian.Game.Map.Base.prototype.render.call(this, h);
            this.element.addClass("view").css({
                position: "absolute",
                zIndex: 3
    ...
    this.render = function(c) {
        ...
        if (d.length > 0) {
            this.viewFull = d.on("click", function(g) {
                window.location.href = Travian.Helpers.substitute(f.viewFullScreenUrl, Object.assign({}, f.mapContainer.transition.getPointOfCenterInView(), {
        ...
        if (d.length > 0) {
            this.viewNormal = this.element.find(".iconButton.viewNormal").on("click", function(g) {
                window.location.href = Travian.Helpers.substitute(f.viewNormalUrl, Object.assign({}, f.mapContainer.transition.getPointOfCenterInView(), {
                    zoom: f.mapContainer.transition.zoomLevel
                }))
            })
        }
...
Travian.Game.Map.Layer.Symbol.Flag = function(a, b) {
    ...
    this.updateData = function(c) {
        ...
        this.element.set("src", this.imgSource.substitute(Object.assign({}, this.parameters, {
            zoomLevel: this.transition.zoomLevel,
            width: this.symbolSize.width,
            height: this.symbolSize.height
        })))
...
Travian.Game.Map.MapMark = function(a, b) {
    ...
    this.update = function() {
        ...
        for (var c = 0; c < g.length; c++) {
            ...
            h.expandContainer.css({
                height: h.expanded ? f : 0
            })
        }
    };
    Travian.Game.Map.Base.call(this, a, b);
...
Travian.Game.Map.MapMark.Layer.Mark = function(a, c) {
    ...
    this.addData = function(d) {
        var e = new Travian.Game.Map.MapMark.Layer.Data.Mark(Object.assign({}, d, this.optionsData, {
...
Travian.Game.Map.MapMark.Layer.Flag = function(a, c) {
    ...
    this.addData = function(e) {
        var d = new Travian.Game.Map.MapMark.Layer.Data.Flag(Object.assign({}, e, this.optionsData, {
            editable: this.editable
        }), this);
        this.datas[d.id] = d;
        return this
    ...
    for (var b = this.indexes.min; b <= this.indexes.max; b++) {
        this.imagesArray.push(Travian.Helpers.substitute(this.imgSource, {
            index: b
        }))
    }
};
...
Travian.Game.Map.MapMark.Layer.Data.Flag = (function() {
    var a = function(c) {
        ...
        c.elements.index.html('<img src="' + Travian.Helpers.substitute(c.parentContainer.imgSource, {
            index: c.index
        }) + '" alt="" />')
    };
    var b = function(c, d) {
        ...
        this.del = function() {
            ...
            Travian.ajax({
                ...
                onSuccess: function(f) {
                    if (f.result) {
                        e.destroy();
                        e.dataStore.remove(Travian.Game.Map.DataStore.TYPE_SYMBOL, {
                            x: e.x,
                            y: e.y
                        }, e.dataId);
            ...
            Travian.ajax({
                ...
                onSuccess: function(h) {
                    if (h) {
                        ...
                        f.text = g;
                        a(f);
                        var i = f.dataStore.get(Travian.Game.Map.DataStore.TYPE_SYMBOL, {
                            x: f.x,
                            y: f.y
                        }, f.dataId);
                        i.index = f.index;
